...
const getIcon = (type) => {
  switch(type) {
    case 'Підключення': return <Cable className="w-6 h-6 md:w-8 md:h-8 text-red-600" />;
    case 'Ремонт': return <Wrench className="w-6 h-6 md:w-8 md:h-8 text-red-600" />;
    case 'Додаткові послуги': return <Radio className="w-6 h-6 md:w-8 md:h-8 text-red-600" />;
    case 'Магазин': return <ShoppingCart className="w-6 h-6 md:w-8 md:h-8 text-red-600" />;
    default: return null;
  }
};
...
const EmptyState = () => (
  <div className="flex flex-col items-center justify-center p-8 bg-gray-900 rounded-xl border-2 border-dashed border-gray-700 mt-4">
...
const MyTaskViewer = () => {
  ...
  if (error) {
    return (
      <div className="container mx-auto p-4">
        <div className="bg-gray-900 border border-red-600 rounded-lg p-4 text-red-500">
          {error}
        </div>
      </div>
    );
  ...
  const renderTaskContent = (task) => {
    if (task.type === 'Магазин') {
      ...
      return (
        <>
          <p className={`text-sm sm:text-lg ${task.avaible ? 'text-green-500' : 'text-red-500'} font-medium mb-1 sm:mb-2`}>
        ...
        {filteredTasks.map((task) => (
          <div 
            ...
            className="
              ...
              hover:shadow-lg border border-gray-800
            "
          >
            <div className="flex items-center justify-between mb-2 sm:mb-4">
              <span className="text-lg sm:text-2xl font-bold text-red-600">#{task.id}</span>
